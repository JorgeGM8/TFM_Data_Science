...
tab1, tab2 = st.tabs(['🔍 Análisis', '🔮 Predicciones'])
...
with tab1:
    ...
    tab3, tab4, tab5 = st.tabs(["🔍 Vista Completa", "📊 Resumen por Distrito", "💾 Descargar"])
...
with tab2:
    ...
    MODEL_PATH_VENTA = "models/lgbm_final_venta.pkl"
    MODEL_PATH_ALQUILER = "models/lgbm_final_alquiler.pkl"

    @st.cache_resource(show_spinner=True)
    def load_models():
        with open(MODEL_PATH_VENTA, "rb") as f:
            modelo_venta = pickle.load(f)
        with open(MODEL_PATH_ALQUILER, "rb") as f:
    ...
    keys = ["Planta", "Tamano", "Ano", "Distrito", "Operacion"]
    st.json({k: input_dict.get(k) for k in keys})
    st.caption("El resto de variables han sido ajustadas automáticamente.")
    ...
    def predecir_precio_vivienda(vivienda:dict, df:pd.DataFrame, tendencias_socio:pd.DataFrame, modelo):
        ...
        df_dist = df.copy()
        # Cálculos de volatilidad, ratio de precio vs distrito y precio promedio de distrito
        agg = (
        df_dist.groupby(['Distrito', 'Ano'])['Precio_ajustado']
        .agg(['mean', 'std'])
        ...
        df_dist = df_dist.merge(agg, on=['Distrito', 'Ano'], how='left')
        
        df_dist = df_dist[df_dist['Distrito'] == normaliza_distrito(distrito)]
        ...
        fila_base['ratio_vs_distrito_ano_ant'] = (df_dist['Precio_ajustado'].iloc[-1] / df_dist['precio_prom_distrito_ano_ant'].iloc[-1])
        fila_base['volatilidad_distrito_ano_ant'] = df_dist['volatilidad_distrito_ano_ant'].iloc[-1]
        fila_base['precio_prom_distrito_ano_ant'] = df_dist['precio_prom_distrito_ano_ant'].iloc[-1]
        ...
        for var in vivienda.keys():
            if var not in ['Tipo_vivienda', 'Distrito', 'Operacion']: # que no sean dummies
                ...
                if var == 'Distrito':
                    for distrito_norm in [normaliza_distrito(x) for x in distritos_visibles if normaliza_distrito(x) != 'ARGANZUELA']:
                        col_name = f'Distrito_{distrito_norm}'
                        fila_base[col_name] = 1 if distrito_norm == vivienda[var] else 0
        ...
        FEATURES = ['Ano', 'Esperanza_vida', 'Mayores_65anos%', 'Menores_18anos%',
        ...
        'precio_prom_distrito_ano_ant', 'volatilidad_distrito_ano_ant',
        'ratio_vs_distrito_ano_ant', 'Distrito_BARAJAS', 'Distrito_CARABANCHEL',
        'Distrito_CENTRO', 'Distrito_CHAMARTIN', 'Distrito_CHAMBERI',
        'Distrito_CIUDADLINEAL', 'Distrito_FUENCARRALELPARDO',
        'Distrito_HORTALEZA', 'Distrito_LATINA', 'Distrito_MONCLOAARAVACA',
        'Distrito_MORATALAZ', 'Distrito_PUENTEDEVALLECAS', 'Distrito_RETIRO',
        'Distrito_SALAMANCA', 'Distrito_SANBLASCANILLEJAS', 'Distrito_TETUAN',
        'Distrito_USERA', 'Distrito_VICALVARO', 'Distrito_VILLADEVALLECAS',
        'Distrito_VILLAVERDE', 'Operacion_venta', 'Tipo_vivienda_chalet',
        'Tipo_vivienda_dúplex', 'Tipo_vivienda_estudio', 'Tipo_vivienda_loft',
        'Tipo_vivienda_mansión', 'Tipo_vivienda_tríplex',
        'Tipo_vivienda_ático']
    ...
    def predecir_hasta_ano(vivienda: dict, df: pd.DataFrame, tendencias_socio: pd.DataFrame, modelo):
        """
        Extiende la predicción año a año desde el último año con datos reales
        hasta el año objetivo indicado en vivienda['Ano'].
        Usa la función predecir_precio_vivienda() existente sin modificarla.
        """
        ...
        for ano in range(ultimo_ano + 1, ano_objetivo + 1):
            vivienda_iter = vivienda.copy()
            vivienda_iter['Ano'] = ano

            precio_pred = predecir_precio_vivienda(vivienda_iter, df_pred, tendencias_socio, modelo)

            nueva_fila = {
                'Distrito': vivienda_iter['Distrito'],
                'Ano': ano,
                'Precio_ajustado': precio_pred
            }

            df_pred = pd.concat([df_pred, pd.DataFrame([nueva_fila])], ignore_index=True)
    ...
    if ejecutar_prediccion:
        try:
            ...
            if input_dict["Operacion"] == 'venta':
                precio_pred = predecir_hasta_ano(input_dict, df_modelado, df_tendencias, modelo_venta)
                st.success(f"💰 Precio estimado: **{precio_pred:,.0f} €**")
            else:
                precio_pred = predecir_hasta_ano(input_dict, df_modelado, df_tendencias, modelo_alquiler)
                st.success(f"💰 Precio estimado: **{precio_pred:,.0f} €/mes**")
